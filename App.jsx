...
═══════════════════════════════════════════ */
const getLevelSets = (level, base) => {
  const m = { debutant:0.75, intermediaire:1, avance:1.25 };
  const reps = { debutant:"12–15", intermediaire:"8–12", avance:"6–8" };
  const rest = { debutant:"90s", intermediaire:"75s", avance:"60s" };
  const sets = Math.round(base * (m[level]||1));
  return `${sets}×${reps[level]||"8–12"} (repos ${rest[level]||"75s"})`;
};
